...
function createRawss(rootElement) {
    var resolvers = [];
    var engine = engine_1.create(rootElement);
    function once() {
        engine.run(resolvers);
    }
    function resolve(mutations) {
        var relevantMutations = mutations.filter(function (m) { return !engine.isManaging(m); });
    ...
    return {
        ...
        once: once, start: start, pause: pause,
        settle: function () {
            return engine.waitForStylesToBeLoaded();
        }
    };
}
exports.createRawss = createRawss;
...
Object.defineProperty(exports, "__esModule", { value: true });
var rawss_1 = __webpack_require__(15);
/**
 * @hidden
 */
...
function cssVariables(options) {
    if (options === void 0) { options = { rootElement: window.document.documentElement, rawss: null, prefix: '--' }; }
    var rawss = options.rawss || rawss_1.createRawss(options.rootElement || window.document.documentElement);
    var prefix = escapeRegExp(options.prefix);
    var DECLARE_REGEX = new RegExp(prefix + "\\w+$");
    var USE_REGEX = new RegExp("var\\(\\s*" + prefix + "(\\w+)\\s*\\)");
    function resolve(element, getRawStyle) {
        var style = getRawStyle(element);
        return Object.keys(style).reduce(function (newStyle, key) {
            var value = style[key];
            var index = 0;
            var accumStyle = newStyle;
            var match = null;
            while (match = USE_REGEX.exec(value.substr(index))) {
                var varName = match[1];
                var attrName = options.prefix + varName;
                var resolvedValue = style[attrName];
                if (!resolvedValue) {
                    for (var e = element.parentElement; e && !resolvedValue; e = e.parentElement) {
                ...
                accumStyle = __assign({}, accumStyle, (_a = {}, _a[key] = value, _a));
...
Object.defineProperty(exports, "__esModule", { value: true });
var cssvar_1 = __webpack_require__(16);
exports.cssVariables = cssvar_1.cssVariables;
